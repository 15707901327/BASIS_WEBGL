...
var FSHADER_SOURCE =
    ...
    'vec3 smoothcolor(float s,float t,float s_mid,float s_width,float t_mid,float t_width,float blur,vec3 beginRGB,vec3 endRGB){\n' +

    '  float s_min = s_mid - s_width/2.0;\n' +
    '  float s_max = s_mid + s_width/2.0;\n' +

    '  float t_min = t_mid - t_width/2.0;\n' +
    '  float t_max = t_mid + t_width/2.0;\n' +

    '  float blend = smoothstep(s_min, s_min + blur, s) * (1.0 - smoothstep(s_max - blur, s_max, s)) * smoothstep(t_min, t_min + blur, t) * (1.0 - smoothstep(t_max - blur, t_max, t));\n' +
    '  return mix(beginRGB, endRGB, blend);\n' +
    '}\n' +

    'void main() {\n' +
    '  float s = 1.0 - (v_Position.x + 0.5);\n' +
    '  float t = 1.0 - (v_Position.y + 0.5);\n' +
    '  vec3 color = smoothcolor(s,t, 0.5,0.7, 0.5,0.7, 0.1, vec3(0.6,0.6,0.0),vec3(0.6,0.121,0.0));\n' +
